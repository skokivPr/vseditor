...
function setTheme(theme, customTheme = null) {
    ...
    if (editor) {
        ...
        if (customTheme === 'dragan') {
            ...
            // Paleta kolorów kodu jak cyber-code (terminal-dark / terminal-light z monaco-style.js)
            monacoTheme = isDarkMode ? 'terminal-dark' : 'terminal-light';
        }
        ...
        if (editorElement) {
            if (customTheme === 'dragan') {
                ...
                // Odśwież motyw terminal (kolory jak cyber-code)
...
function getDefaultEditorOptions() {
    ...
    const fontFamily = settings.appearance.fontFamily?.default ?? settings.appearance.fontFamily;
    const fontSize = p.fontSize ?? settings.appearance.fontSize?.default ?? settings.appearance.fontSize ?? 14;
    const lineHeight = settings.appearance.lineHeight?.default ?? settings.appearance.lineHeight ?? 21;
    const letterSpacing = settings.appearance.letterSpacing?.default ?? settings.appearance.letterSpacing ?? 0;
    const guidesOn = p.indentGuides !== false;
    const lineHeightPref = Math.max(16, Math.min(32, parseInt(String(p.lineHeight ?? lineHeight), 10) || 21));

    return {
        value: getDefaultContent(),
        language: 'html',
        theme: isDarkMode ? 'terminal-dark' : 'terminal-light',
        fontFamily,
        fontSize,
        ...
        tabSize: parseInt(String(p.tabSize ?? settings.indentation.tabSize.default), 10) || 4,
        insertSpaces: p.insertSpaces !== false,
        cursorBlinking: p.cursorBlinking || settings.advanced.cursorBlinking.default,
        cursorStyle: p.cursorStyle || settings.advanced.cursorStyle.default,
...
function initializeEditor() {
    ...
    // Wzorzec jak w cyber-code (https://cdn.jsdelivr.net/gh/s-pro-v/cyber-code@main/index.js):
    // defineMonacoThemes + configureMonacoCssSupport zaraz po editor.main, przed utworzeniem edytora.
    if (typeof defineMonacoThemes === 'function') {
        defineMonacoThemes();
    }
    if (typeof configureMonacoCssSupport === 'function') {
...
/** CDN `min/vs` — ten sam układ co cyber-code (loader w index.html, require tutaj). */
const CYBER_MONACO_VS = 'https://cdnjs.cloudflare.com/ajax/libs/monaco-editor/0.49.0/min/vs';

function startMonacoFromLoader() {
    if (typeof require === 'undefined') {
        console.error(
            'Monaco: brak AMD `require`. W <head> dodaj: vs/loader.js (patrz cyber-code index.html).'
        );
        return;
    }
    require.config({ paths: { vs: CYBER_MONACO_VS } });
    require(
        ...
        function (err) {
            console.error('Monaco: nie załadowano vs/editor/editor.main', err);
        }
    );
}
...
if (typeof monaco !== 'undefined' && monaco.editor && typeof monaco.editor.create === 'function') {
    ...
    if (document.readyState === 'loading') {
        document.addEventListener('DOMContentLoaded', startMonacoFromLoader);
    } else {
        startMonacoFromLoader();
    }
} else {
    console.error('Monaco: brak loadera (vs/loader.js) oraz brak gotowego `monaco`.');
}
...
function setBracketGuidesEnabled(enabled, showStatus = false) {
    ...
    saveEditorPrefs({ indentGuides: bracketGuidesEnabled });
    if (showStatus) {
        updateFooterStatus(
            bracketGuidesEnabled ? 'Przewodniki włączone' : 'Przewodniki wyłączone',
            'code'
        );
        setTimeout(() => updateFooterStatus('Ready', 'circle'), 2000);
    }
...
function toggleBracketGuides() {
    if (!editor) return;
    setBracketGuidesEnabled(!bracketGuidesEnabled, true);
}
...
// VS Code theme helpers (also used from HTML / debugging)
...
function loadSavedTheme() {
    ...
    if (savedCustomTheme === 'dragan') {
        ...
        // terminal-dark / terminal-light — już zarejestrowane w defineMonacoThemes()
        setTheme(savedTheme);
        console.log(
            '🎨 Motyw terminal (cyber):',
            savedTheme === 'dark' ? 'terminal-dark' : 'terminal-light'
        );
    }

    // Update footer status immediately
    const themeDisplay =
        savedCustomTheme === 'dragan'
            ? 'Dragan'
            : savedTheme === 'dark'
                ? 'Terminal (ciemny)'
                : 'Terminal (jasny)';
...
function newFile() {
    if (!editor) {
        showCustomModal({
            ...
            text: 'Please wait until the editor finishes loading, then try again.',
            timer: 2500,
            showConfirmButton: false
        });
        return;
...
function showCustomModal(options) {
    ...
    // Create modal HTML
    const modalHTML = `
        <div class="custom-modal-overlay" id="customModal" role="dialog" aria-modal="true" aria-labelledby="customModalTitle">
            <div class="custom-modal-content">
                <div class="custom-modal-header">
                    <div class="custom-modal-icon ${icon}" aria-hidden="true">
                ...
                <div class="custom-modal-actions">
                    ${showCancelButton ? `<button type="button" class="custom-modal-btn cancel-btn" id="modalCancel">${cancelButtonText}</button>` : ''}
                    ${showConfirmButton ? `<button type="button" class="custom-modal-btn confirm-btn" id="modalConfirm">${confirmButtonText}</button>` : ''}
                </div>
            </div>
        </div>
    `;
    ...
    return new Promise((resolve) => {
        ...
        if (timer) {
            setTimeout(() => {
                if (!settled && document.getElementById('customModal')) {
                    settle(false);
                }
            }, timer);
        }
...
function createInfoModal() {
    const modalHTML = `
        <div id="infoModal" class="modal-overlay info-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="infoModalTitle" tabindex="-1">
            <div class="modal-content info-modal-content">
                <div class="modal-header info-modal-header">
                    <h2 id="infoModalTitle"><i class="fas fa-info-circle" aria-hidden="true"></i> Pomoc i informacje</h2>
                    <button type="button" class="modal-close info-modal-close-icon" aria-label="Zamknij">
                        <i class="fas fa-times" aria-hidden="true"></i>
                    </button>
                </div>
                <div class="modal-body info-modal-body">
                    <div class="info-section">
                        <h3><i class="fas fa-keyboard" aria-hidden="true"></i> Skróty klawiszowe</h3>
                        <p class="info-modal-shortcuts-note">W opisach <strong>Ctrl</strong> oznacza też <strong>⌘ (Cmd)</strong> na macOS — te skróty są obsługiwane przez stronę.</p>
                        <h4 class="info-shortcuts-sub">Ta aplikacja</h4>
                        <div class="shortcuts-grid">
                            <div class="shortcut-item">
                                <span class="shortcut-key">Esc</span>
                                <span class="shortcut-desc">Zamknij panel ustawień (gdy otwarty) lub ten modal pomocy</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">F1</span>
                                <span class="shortcut-desc">Otwórz / przełącz ten modal pomocy</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + ,</span>
                                <span class="shortcut-desc">Panel ustawień edytora (otwórz / zamknij)</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + N</span>
                                <span class="shortcut-desc">Nowy plik</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + O</span>
                                <span class="shortcut-desc">Otwórz plik z dysku</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + S</span>
                                <span class="shortcut-desc">Zapisz plik (pobranie)</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + Enter</span>
                                <span class="shortcut-desc">Uruchom / odśwież podgląd HTML</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + T</span>
                                <span class="shortcut-desc">Przełącz motyw jasny / ciemny strony</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Alt + Shift + F</span>
                                <span class="shortcut-desc">Formatuj kod (Prettier, gdy dostępne)</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + .</span>
                                <span class="shortcut-desc">Zatrzymaj ładowanie podglądu</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + Shift + B</span>
                                <span class="shortcut-desc">Włącz / wyłącz przewodniki wcięć w edytorze</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">Ctrl + Shift + L</span>
                                <span class="shortcut-desc">Napraw układ Monaco (layout po zmianie rozmiaru)</span>
                            </div>
                            <div class="shortcut-item">
                                <span class="shortcut-key">F11</span>
                                <span class="shortcut-desc">Pełny ekran przeglądarki</span>
                            </div>
                        </div>
                        <h4 class="info-shortcuts-sub">Edytor Monaco (jak w VS Code)</h4>
                        <p class="info-monaco-shortcuts">Te skróty obsługuje sam edytor, nie strona: <strong>Ctrl+F</strong> szukaj, <strong>Ctrl+H</strong> zamień, <strong>Ctrl+Z</strong> / <strong>Ctrl+Y</strong> cofnij i ponów, <strong>Ctrl+/</strong> komentarz linii, <strong>Ctrl+Spacja</strong> podpowiedzi, <strong>Alt+↑/↓</strong> przesuń linię, <strong>Alt+klik</strong> wiele kursorów, <strong>Ctrl+D</strong> następne wystąpienie, <strong>Ctrl+G</strong> skok do linii, <strong>Ctrl+K Ctrl+C</strong> zakomentuj zaznaczenie (akord dwóch kroków). Pełną listę masz w menu kontekstowym i w palecie poleceń Monaco.</p>
                    </div>

                    <div class="info-section">
                        <h3><i class="fas fa-tools" aria-hidden="true"></i> Funkcje</h3>
                        <ul class="features-list">
                            <li><strong>Monaco Editor:</strong> edycja jak w VS Code — składnia, skróty, menu kontekstowe</li>
                            <li><strong>Języki:</strong> HTML, CSS, JavaScript, Python, TypeScript, Java, C#, PHP</li>
                            <li><strong>Podgląd na żywo:</strong> dla HTML w osobnym panelu</li>
                            <li><strong>Formatowanie:</strong> Prettier dla HTML, CSS i JavaScript</li>
                            <li><strong>Motywy:</strong> jasny / ciemny, motywy tokenów (m.in. terminal-dark)</li>
                            <li><strong>localStorage:</strong> automatyczny zapis pracy (stan edytora)</li>
                            <li><strong>Pliki:</strong> nowy, otwórz, zapisz do pobrania</li>
                            <li><strong>Widok:</strong> podział, tylko edytor, tylko podgląd</li>
                            <li><strong>Ustawienia:</strong> panel boczny — przycisk na pasku lub <strong>Ctrl+,</strong> (⌘+, na Mac); <strong>F1</strong> otwiera pomoc</li>
                            <li><strong>Błędy składni:</strong> podświetlanie w edytorze</li>
                            <li><strong>Kolory w kodzie:</strong> podgląd dekoracji dla hex, RGB, HSL</li>
                            <li><strong>Przewodniki:</strong> włącz / wyłącz linie wcięć</li>
                        </ul>
                    </div>

                    <div class="info-section">
                        <h3><i class="fas fa-lightbulb" aria-hidden="true"></i> Wskazówki</h3>
                        <ul class="tips-list">
                            <li>Gdy wisi modal potwierdzenia, skróty strony (N, O, S itd.) są wyłączone — najpierw potwierdź lub anuluj.</li>
                            <li>Na Macu używaj <strong>⌘</strong> zamiast <strong>Ctrl</strong> w skrótach aplikacji (N, O, S, Enter, T, przecinek itd.).</li>
                            <li><strong>F1</strong> w przeglądarce czasem jest przejmowane przez system — wtedy użyj przycisku pomocy na pasku.</li>
                            <li>Prawy przycisk w edytorze — menu kontekstowe Monaco (więcej poleceń niż na liście).</li>
                            <li>Stan edytora zapisuje się w <strong>localStorage</strong> tej przeglądarki.</li>
                            <li>Język pliku zmieniasz listą nad edytorem — model i podświetlanie idą za wyborem.</li>
                            <li>Po zmianie rozmiaru okna, jeśli edytor „pływa”, spróbuj <strong>Ctrl+Shift+L</strong> (⌘+Shift+L).</li>
                        </ul>
                    </div>

                    <div class="info-section info-section--last">
                        <h3><i class="fas fa-book" aria-hidden="true"></i> O programie</h3>
                        <p class="info-modal-lead">Edytor w przeglądarce oparty na <strong>Monaco Editor</strong> (ten sam silnik co w VS Code): podświetlanie składni, IntelliSense i podgląd HTML.</p>
                        <p class="info-modal-meta"><strong>Wersja:</strong> 2.1.0</p>
                        <p class="info-modal-meta"><strong>Stack:</strong> Monaco 0.49, Prettier, modale i panel ustawień (styl HUD / cyber-code)</p>
                    </div>
                </div>
                <div class="info-modal-footer">
                    <button type="button" class="info-modal-close-btn">Zamknij</button>
                </div>
            </div>
        </div>
    `;
...
function showInfoModal() {
    ...
    if (!modal) {
        ...
        modal.addEventListener('click', function (e) {
            ...
            if (e.target.closest('.modal-close') || e.target.closest('.info-modal-close-btn')) {
                hideInfoModal();
            }
        });
    }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('keydown', function (e) {
        ...
        // Only process shortcuts if help modal is not open
        if (modal && modal.classList.contains('show')) {
            return;
        }

        // Ctrl/Cmd + , — panel ustawień (toggle, jak w VS Code)
        if (mod && e.key === ',') {
            e.preventDefault();
            const sb = document.getElementById('settings-sidebar');
            if (sb && sb.classList.contains('active')) {
        ...
        // Ctrl/Cmd + Enter — podgląd (skrót jak w cyber-code)
        ...
        // Ctrl/Cmd + T — przełącz motyw (jak w cyber-code)
        ...
        // Ctrl/Cmd + . - Stop preview
        if (mod && e.key === '.') {
            e.preventDefault();
            stopPreview();
        }

        // Ctrl/Cmd + Shift + B - Toggle Bracket Guides
